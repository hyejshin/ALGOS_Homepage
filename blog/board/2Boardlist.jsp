...
<BODY>
<H1>2학년 문제 게시판</H1><br>
<a href=index.jsp?contentPage=board/writeForm.jsp?board=2>글등록</a>
...
<%
	try{
		...
		if(lastRow > 0) {
			strSQL = "SELECT * FROM board WHERE grade='2학년' order by pidx DESC";
...
%>
				<TR>
					...
					<a href=index.jsp?contentPage=board/view.jsp?idx=<%=rs.getInt("idx")%>&pageNum=<%=pageNum%>&board=2><%=rs.getString("subject")%></a></TD>
...
<%
	if(lastRow > 0) {
		...
		if(currentPage > 1) {
%>
		<a href="index.jsp?contentPage=board/2Boardlist.jsp?pageNum=<%=currentPage-1%>">[이전]</a>
<%
		}
		// 레코드 수에 따라 쪽번호를 매긴다.	
		while(setPage <= lastPage) {
%>
				<a href="index.jsp?contentPage=board/2Boardlist.jsp?pageNum=<%=setPage%>">[<%=setPage%>]</a>
<%
			setPage = setPage + 1;
		}
...
%>
		<a href="index.jsp?contentPage=board/2Boardlist.jsp?pageNum=<%=currentPage+1%>">[다음]</a>
<%
		}
	}
%>
