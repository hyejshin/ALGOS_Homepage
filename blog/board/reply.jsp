...
<body>
<h2>답글 등록</h2><br>
	<form id="my_form" action="index.jsp?contentPage=board/reply_ok.jsp?board=<%=board%>" method="post" onsubmit="return check();">
	<table width="750">
        <tr><td class="head">카테고리</td>
            <td><select name="grade" disabled><option value="<%=board%>학년"><%=board%>학년</option><option value="2학년">2학년</option>
                <option value="3">3학년</option><option value="4">4학년</option>
                </select>
                <select name="accept" disabled><option value="<%=accept%>"><%=accept%></option>
	...
	<input type="hidden" name="grade" value=<%=board%>>
