...
<body>
	...
	if(rs.next()){
	%>
	<%=id %>는 이미  <font color="red">사용중</font>입니다.<br/>
	다시 입력하세요.<br/>
		<input type="button" value="돌아가기" onClick="javascript:checkClose('<%=id%>')" >
	<%
	}else{
	%>
		<%=id %> 사용 <font color="red">가능</font><br/>
		<input type="button" value="현재 아이디 선택" onClick="javascript:checkClose('<%=id%>')" >
	<%
	}
	%>
